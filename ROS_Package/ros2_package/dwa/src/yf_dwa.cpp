...
public:
      ...
      : Node("DWA_Subscriber")
  {
    ...
    subscriptionMaplist_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(
        "/yf_camera/obmap", 1, std::bind(&DWA_Subscriber::topicOblist_callback, this, _1));
...
private:
  ...
  void topicOblist_callback(const std_msgs::msg::Float32MultiArray::SharedPtr mapmsg) const
  {
    ...
    float *v2 = v.data();
    Map<MatrixXf> ob(v2, mapmsg->data.size() / 2, 2);
    oblist = ob.transpose().cast<double>();
  }

  void topicGoal_callback(const geometry_msgs::msg::PoseStamped::SharedPtr message) const
  {
    Goal temp_goal;
    temp_goal << message->pose.position.x + 0.06, message->pose.position.y;
    goal = temp_goal; 
  }
...
public:
  ...
  {
    subscriptionSpeed_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(
        "dwa_sollspeed", 1, std::bind(&DWA_RealspeedSubscriber::topic_callback, this, _1));
  }
...
public:
  ...
  {
    subscriptionSpeed_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(
        "dwa_sollspeed", 1, std::bind(&DWA_Visualizer::topic_callback, this, _1));
  }
...
private:
  void topic_callback(const std_msgs::msg::Float32MultiArray::SharedPtr msg) const
  {
    ...
    if (planner.SHOW_ANIMATION)
    {
      ...
      cv::circle(bg, cv_offset(car_x(0), car_x(1), bg.cols, bg.rows), int(scale_up * 0.5), cv::Scalar(0, 0, 255), 2);
      cv::arrowedLine(bg, cv_offset(car_x(0), car_x(1), bg.cols, bg.rows),
                      cv_offset(car_x(0) + std::cos(car_x(2)), car_x(1) + std::sin(car_x(2)), bg.cols, bg.rows),
                      cv::Scalar(255, 0, 255),
                      int(figure_size / 300));
