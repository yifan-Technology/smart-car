...
public:
	float max_speed = 2.0;
	float min_speed = -0.5;
	float max_yawrate = 180.0 * PI / 180.0;
	float max_accel = 5;
	float robot_radius = 1.0;
	float max_dyawrate = 180.0 * PI / 180.0;

	float v_reso = 0.1;
	float yawrate_reso = 1 * PI / 180.0;
...
int main() {
	...
	for (int i = 0; i < 1000 && !terminal; i++) {
		...
		std::cout << "Elapsed time is:" << elapsed_time << " (s)" << endl;
		count++;
	}
	...
	for (int i = 0; i < traj.size(); i++) {
		std::cout << "state: " << fixed << setprecision(2) << traj[i][0] << "," << traj[i][1] << endl;
		state_cout++;
	}
	std::cout << "total step in trajectory: " << state_cout << endl;
	return 0;
}