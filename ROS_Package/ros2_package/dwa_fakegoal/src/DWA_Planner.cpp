...
namespace dwa_planner {
	...
	double DWA::calc_obstacle_cost(MatrixXd trajectory, MatrixXd obstacle, double v0) {
		/*"""
	...
	DWA_result DWA::calc_control_and_trajectory(State state, Window dw, Goal goal, MatrixXd obstacle) {
		...
		vector<double> v_temp, omega_temp, v_range, omega_range, v_range0, omega_range0, omega_back;

		// generate quick turning range 
		...
		omega_back.push_back(1.5 * PI);
		omega_back.push_back(-1.5 * PI);

		// prefer turning around
		double goal_theta = atan2(goal(1), goal(0)) * 180 / PI;
		bool  shouldTurning = false;

		if (!FAKE_GOAL) {  // normal goal and normal dwa
		...
		if (shouldTurning) {
			FAKE_GOAL = true;
			goal_before_fake = goal;
			v_temp = v_range0;
			omega_temp = omega_range0;
			cout << "should turing state:" << shouldTurning << endl;
		}
		else {
			// small algorithm
			if (goal_theta > 135 || goal_theta < 45) {
		...
		for (auto v : v_temp) {
			...
			for (auto omega : omega_temp) {
				...
				double dist_square = calc_obstacle_cost(predict_traj, obstacle, v);
				...
				//double speed_cost = 0.5 * (max_speed - predict_traj(3, predict_traj.cols() - 1));
	...
	DWA_result DWA::dwa_control(Control motor_ist, State x_pre, Goal zw_goal, MatrixXd ob_list) {
		...
		//cout<<"x_trans:"<<x_trans<<endl;S
		if (RESET_STATE) {
			temp_x.topRows<3>() << 0, -0.3, PI / 2;
			temp_x.bottomRows<2>() = x_trans.bottomRows<2>();
		}
		if (FAKE_GOAL) {
			if (goal_before_fake(0)>=0) {
				temp_goal <<3 - count_time * 3 / 80, 4;
			}
			else {
				temp_goal << 3 - count_time * 3 / 80, 4;
			}
		...
		cout<<"zw goal:"<<zw_goal.transpose()<<endl;
		cout << "temp goal:" << temp_goal.transpose() << endl;
		Window dw = calc_dynamic_window(temp_x);
		DWA_result dwa_result = calc_control_and_trajectory(temp_x, dw, temp_goal, ob_list);
		...
		if (dist_to_goal <= robot_radius + 1.2) {
			predict_time = 0.8;
			max_accel = 2;
		}
		else {
			predict_time = predict_time0;
			max_accel = max_accel0;
		}
