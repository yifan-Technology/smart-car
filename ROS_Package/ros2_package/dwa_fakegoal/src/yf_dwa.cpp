...
public:
	...
	{
		publisherSollspeed_ = this->create_publisher<std_msgs::msg::Float32MultiArray>("soll_speed", 1);
...
public:
		...
		: Node("DWA_Parametrize")
	{
		...
		this->declare_parameter<double>("safe_radius", 0.8);
		...
		this->declare_parameter<bool>("PRINT_COST", true);
		this->declare_parameter<bool>("PLOT_ESTIMATE_TRAJ", false);
		timer_ = this->create_wall_timer(
			1000ms, std::bind(&DWA_Parametrize::respond, this));
	}
	void respond()
	{
		...
		this->get_parameter("safe_radius", planner.safe_radius);
		...
		planner.max_accel0 = planner.max_accel;
...
public:
		...
		: Node("DWA_Visualizer"), count_(0)
	{
		...
		timer_ = this->create_wall_timer(
			40ms, std::bind(&DWA_Visualizer::timer_callback, this));
	}
...
int main(int argc, char** argv)
{
	...
	planner.temp_ucal << 0, 0;
	planner.STOP_TURNING = false;
	goal << 0., 0.1;
	fakegoal << 0., 3;
	MatrixXd obin(20, 2);
	obin << -1, -1,
