...
namespace dwa_planner {
	...
	//	}
	MatrixXd DWA::traj_W2B_transformation(Pose carpose, MatrixXd solltraj) {
		double theta = carpose(2) - pose_bias(2);//- PI / 2;
		//cout << "theta: " << theta * 180 / PI << endl;
		//		Control bodyTrans = carpose.segment(3,2) - pose_temp.segment(3,2);
		MatrixXd bodyTrans;
		bodyTrans = carpose.segment(3, 2) - pose_bias.segment(3, 2);
		cout<<"current pose:"<<bodyTrans.transpose()<<endl;
		Matrix2d Rot_i2bT;
		Rot_i2bT << cos(theta), sin(theta), -sin(theta), cos(theta);
		return Rot_i2bT * solltraj + bodyTrans.replicate(1, solltraj.cols());
	}
