...
import time
import yaml
from AutoParkingNew import Parking_Controller
...
def main():
    ...
    try:
        while True:
            ...
            #goalGenerator.state = parkplanner.phase

            key = cv2.waitKey(1) 
            if(key== ord('q')) or parkplanner.MovingState == "final_stop": 
                print("尝试退出")
                goalGenerator.zed.close()
                break        
            elif(key == ord('g')) or parkplanner.phase == "Phase: goin": 
                print("切换成进入状态")
